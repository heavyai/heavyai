...
class TestCPUDataNoNulls:
    ...
    def test_create_load_table_no_nulls_sql_execute(self, con, method):
        ...
        gcd = con.get_column_details("test_data_no_nulls")
        name_types = [(x.name, x.type) for x in gcd]
        assert name_types == [
            ('tinyint_', 'TINYINT'),
            ('smallint_', 'SMALLINT'),
            ('int_', 'INT'),
        ...
        df_in.sort_values(
            by=['tinyint_', 'smallint_', 'int_', 'bigint_'], inplace=True
        )
        ...
        assert pd.DataFrame.equals(
            df_in["tinyint_"], df_out["tinyint_"].astype('int8')
        )

        assert pd.DataFrame.equals(
            df_in["smallint_"], df_out["smallint_"].astype('int16')
        )

        assert pd.DataFrame.equals(
            df_in["int_"], df_out["int_"].astype('int32')
        )
        ...
        assert pd.DataFrame.equals(
            df_in["bool_"], df_out["bool_"].astype('bool')
        )  # noqa
    ...
    def test_load_table_text_no_encoding_no_nulls(self, con, method):
        ...
        con.execute(
            """create table test_text_no_encoding (
                       idx integer,
    ...
    def test_load_table_geospatial_no_nulls(self, con, method):
        ...
        con.execute(
            """create table test_geospatial_no_nulls (
                       tinyint_ tinyint,
                       smallint_ smallint,
                       int_ integer,
                       bigint_ bigint,
        ...
        assert all(
            [x.equals_exact(y, 0.000001) for x, y in zip(mpoly_in, mpoly_out)]
        )
        ...
        assert all(
            [x.equals_exact(y, 0.011) for x, y in zip(poly_in, poly_out)]
        )
